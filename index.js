...
var Config = require('leadsauth-utils').Config;
var GrantManager = require('leadsauth-utils').GrantManager;
...
/**
 * Instantiate a LeadsAuth.
 *
 * The `config` and `leadsauthConfig` hashes are both optional.
 *
 ...
 * from the LeadsAuth console to explicitly logout some or all sessions.
 *
 * In all cases, also, authentication through a Bearer authentication
 * header is supported for non-interactive APIs.
 *
 * The `leadsauthConfig` object, by default, is populated by the contents of
 * a `leadsauth.json` file installed alongside your application, copied from
 * the LeadsAuth administration console when you provision your application.
 *
 * @constructor
 *
 * @param      {Object}    config          Configuration for the LeadsAuth connector.
 * @param      {Object}    leadsauthConfig  LeadsAuth-specific configuration.
 *
 * @return     {LeadsAuth}  A constructed LeadsAuth object.
 *
 */
function LeadsAuth (config, leadsauthConfig) {
  // If leadsauthConfig is null, Config() will search for `leadsauth.json`.
  this.config = new Config(leadsauthConfig);
 ...
 * as it provides general wiring for LeadsAuth interaction, without actually
 * causing LeadsAuth to get involved with any particular URL until asked
 * by using `protect(...)`.
 *
 * Example:
 *
 *     var app = express();
 *     var leadsauth = new LeadsAuth();
 *     app.use( leadsauth.middleware() );
 *
 * Options:
 *
 *  - `logout` URL for logging a user out. Defaults to `/logout`.
 *  - `admin` Root URL for LeadsAuth admin callbacks.  Defaults to `/`.
 *
 * @param {Object} options Optional options for specifying details.
 */
LeadsAuth.prototype.middleware = function (options) {
  if (!options) {
 ...
 * LeadsAuth installation to login.  Upon successful login, the user will
 ...
 *    the installed `leadsauth.json` as provisioned within LeadsAuth) that the
 *    user must have in order to proceed.
 *
 * Example
 *
 *     // Users must have the `special-people` role within this application
 *     app.get( '/special/:page', leadsauth.protect( 'special-people' ), mySpecialHandler );
 *
 ...
 *     }
 *
 *     app.get( '/fancy/:page', leadsauth.protect( pants ), myPantsHandler );
 *
 * With no spec, simple authentication is all that is required:
 *
 *     app.get( '/complain', leadsauth.protect(), complaintHandler );
 *
 * @param {String} spec The protection spec (optional)
 */
LeadsAuth.prototype.protect = function (spec) {
 ...
 * for linking authentication information from LeadsAuth to
 * application-maintained user information.
 *
 * The `request.lauth.grant` object contains the relevant tokens
 * which may be inspected.
 *
 * For instance, to obtain the unique subject ID:
 *
 *     request.lauth.grant.id_token.sub => bf2056df-3803-4e49-b3ba-ff2b07d86995
 *
 * @param {Object} request The HTTP request.
 */
LeadsAuth.prototype.authenticated = function (request) {
  // no-op
};
 ...
 * @param {Object} request The HTTP request.
 */
LeadsAuth.prototype.deauthenticated = function (request) {
  // no-op
};
 ...
 *
 * In the event the LeadsAuth middleware decides a user may
 ...
 */
LeadsAuth.prototype.accessDenied = function (request, response) {
  response.status(403);
  response.end('Access denied');
};

/*! ignore */
LeadsAuth.prototype.getGrant = function (request, response) {
  var rawData;
...
LeadsAuth.prototype.storeGrant = function (grant, request, response) {
...
LeadsAuth.prototype.unstoreGrant = function (sessionId) {
  if (this.stores.length < 2) {
...
LeadsAuth.prototype.getGrantFromCode = function (code, request, response) {
  if (this.stores.length < 2) {
...
LeadsAuth.prototype.loginUrl = function (uuid, redirectUrl) {
...
LeadsAuth.prototype.logoutUrl = function (redirectUrl) {
  return this.config.realmUrl +
  '/protocol/openid-connect/logout' +
  '?redirect_uri=' + encodeURIComponent(redirectUrl);
};

LeadsAuth.prototype.accountUrl = function () {
  return this.config.realmUrl + '/account';
};

LeadsAuth.prototype.getAccount = function (token) {
  return this.grantManager.getAccount(token);
};

LeadsAuth.prototype.redirectToLogin = function (request) {
  return !this.config.bearerOnly;
};

module.exports = LeadsAuth;
